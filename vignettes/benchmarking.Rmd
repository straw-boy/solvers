---
title: "benchmarking"
author: "Akarsh Goyal"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{benchmarking}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r setup}
library(solvers)
```

```{r}
n <- 100
p <- 2
d <- randomProblem(n, p, response = "binomial")

x <- d$x
y <- d$y

fista_fit <- FISTA(x, y, family="binomial", alpha=(0.0001),diagnostics=TRUE)
admm_fit <- ADMM(x, y, family="binomial", opt_algo="nr", alpha=(0.0001),diagnostics=TRUE)

f <- data.frame(
  solver = rep(c("FISTA", "ADMM"), 
               times = c(length(fista_fit$loss), length(admm_fit$loss))),
  loss = c(fista_fit$loss,admm_fit$loss),
  time = c(fista_fit$loss,admm_fit$loss)
)


# x <- scale(xy$x)
# y <- ifelse(xy$y, -1, 1)
# fit <- owl(x, y, scale = FALSE, center = FALSE, family = "binomial")
# i <- 20
# beta <- coef(fit)[, i]
# xbeta <- cbind(1, x) %*% beta
# lambda <- fit$sigma[i]*fit$lambda*nrow(x)
# grad <- t(x) %*% (-y/(1 + exp(y * xbeta)))
# g <- sum(log(1 + exp(-y * xbeta)))
# h <- sum(sort(abs(grad), decreasing = TRUE)*lambda)
# f <- g + h
# max_passes <- 1000
# admm_fit <- admm_binom(x, y, lambda, max_passes, f)
# pn_fit <- pn_binom(x, y, lambda, max_passes, f)
# d <- data.frame(
#   solver = rep(c("ADMM", "PN"), 
#                times = c(length(admm_fit$loss), length(pn_fit$loss))),
#   loss = c(admm_fit$loss, pn_fit$loss),
#   time = c(admm_fit$time, pn_fit$time)
# )
library(lattice)
library(directlabels)
pl <- xyplot(loss ~ time, group = solver, data = f, type = "l")
direct.label(pl, "angled.boxes")
```
---
title: "Poisson Likelihood Benchmarks"
author: "Akarsh Goyal"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Poisson Likelihood Benchmarks}
...
This tutorial documents the performance of the solvers for Poisson Likelihood function.
...
d <- randomProblem(1000, 50, response="poisson", density = 1, rho=0)

# Obtaining fits for each solver and plotting loss vs time graph
f <- getBenchmarks(d$x, d$y, family="poisson")
...
## n < p
```{r, fig.width=10, fig.height=10}
d <- randomProblem(50, 200, response="poisson", density = 1, rho=0)
...
## n > p
```{r, fig.width=10, fig.height=10}
d <- randomProblem(1000, 50, response="poisson", density = 1, rho=0.95)
...
## n < p
```{r, fig.width=10, fig.height=10}
d <- randomProblem(50, 200, response="poisson", density = 1, rho=0.95)
